...
module.exports = {
  ...
  buildImpressions: function (products = [], shouldReduce) {
    ...
    if (!shouldReduce) {
      shouldReduce = product => this.props.version === 'fans' && !product.sold_out;
    }
  ...
  reduceCalloutImpressions: function (genderList, product, i) {
    const calloutImpressions = _.reduce(product.genders, (acc, gender) => {
  ...
  getGenderedDetails: function (gender, id) {
    ...
    const matchedFrame = this.getMatchedFrameData(id);
    const gendered_details = _.find(matchedFrame.gendered_details, {gender: gender});
